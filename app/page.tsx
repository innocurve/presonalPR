          ...
          <section id="profile" className="mb-8 bg-white rounded-xl p-6 sm:p-10 shadow-lg overflow-hidden relative">
            <div className="flex flex-col items-center space-y-6">
              ...
              <div className="text-center">
                <h2 className="text-3xl sm:text-4xl font-bold mb-2">{translate('name', language)}</h2>
                <p className="text-xl sm:text-2xl text-gray-600 mb-4">{translate('title', language)}</p>
              </div>
              <div className="w-full max-w-md space-y-4">
                <ProfileItem label={translate('birth', language)} value={[translate('birthDate', language)]} />
                <ProfileItem 
                ...
                />
                <ProfileItem label={translate('field', language)} value={[translate('fieldDescription', language)]} />
                <ProfileItem label={translate('mbti', language)} value={[translate('mbtiType', language)]} />
              </div>
            </div>
          </section>
        </FadeInSection>
                  ...
                  <Card key={post.id} className="w-full h-full overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300" onClick={() => handlePostClick(post.id)}>
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-end">
                        <span className="text-sm text-gray-500">{translate('views', language)}: {post.views}</span>
                      </div>
                    </div>
                  </Card>
                ))}
...
function ProfileItem({ label, value }: { label: string, value: string[] }) {
  return (
    <div className="mb-4">
      {label && <span className="font-bold text-blue-600 block mb-1 text-lg">{label}</span>}
